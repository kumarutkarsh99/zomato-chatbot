...
export class ChatbotService {
  ...
  // --- Private error handler utility ---
  private handleError(body: any, error: any, handlerName: string) {
    console.error(`Error in ${handlerName}:`, error);
    const session = body.session;
    return {
      fulfillmentText: `I'm sorry, I encountered an error. Please try again. You can say "Delivery" or "Dine-in" to restart.`,
      outputContexts: [
        {
  ...
  private handleNotSupported() {
    return {
      fulfillmentText: `Sorry, that intent is not supported. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
    };
  }
  ...
  private handleWelcome(body: any) {
    try {
      ...
      return {
        ...
        outputContexts: [{ name: `${session}/contexts/awaiting_main_choice`, lifespanCount: 5 }],
      };
    } catch (error) {
      return this.handleError(body, error, 'handleWelcome');
    }
  }
  ...
  private handleDineIn(body: any) {
    try {
      ...
      return {
        ...
        outputContexts: [{ name: `${session}/contexts/dine_in`, lifespanCount: 2 }],
      };
    } catch (error) {
      return this.handleError(body, error, 'handleDineIn');
    }
  }
  ...
  private async handleRestaurantForDine(body: any) {
    try {
      ...
      if (!list.length) {
        return {
          fulfillmentText: `No ${cuisine} restaurants found in ${location}. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
          outputContexts: [
            {
              name: `${session}/contexts/awaiting_main_choice`,
              lifespanCount: 1,
      ...
      return this.handleError(body, error, 'handleRestaurantForDine');
    }
  }
  ...
  private async handleMenuForDine(body: any) {
    try {
      ...
      if (!restaurantName) {
        return {
          fulfillmentText: `Sorry, I didn't catch which restaurant. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
          outputContexts: [
            {
              name: `${session}/contexts/awaiting_main_choice`,
              lifespanCount: 1,
      ...
      if (!items.length) {
        return {
          fulfillmentText: `No menu items found for ${restaurantName}. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
          outputContexts: [
            {
              name: `${session}/contexts/awaiting_main_choice`,
              lifespanCount: 1,
      ...
      return this.handleError(body, error, 'handleMenuForDine');
    }
  }
  ...
  async handleRestaurantDetails(body: any) {
    try {
      ...
      if (!dineCtx || !dineCtx.parameters.restaurantId) {
        return {
          fulfillmentText: `Sorry, I don't know which restaurant you mean. Type "Delivery", "Dine-in", "Track order/dine in" or "Cancel" to continue.`,
          outputContexts: [
            {
              name: `${session}/contexts/awaiting_main_choice`,
              lifespanCount: 1,
      ...
      const details = await this.restaurantService.getById(restaurantId);
      
      const { name, full_address, timing, is_indoor_seating, is_veg_only, dinner_rating, known_for, popular_dishes, people_known_for, cuisines, average_cost } = details;
     ...
     `.trim();
      ...
      return {
       ...
       // Catch specific NotFoundException or any other error
       if (error instanceof NotFoundException) {
            const session = body.session;
            return {
                fulfillmentText: `I couldn't find details for that restaurant. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
                outputContexts: [{
                    name: `${session}/contexts/awaiting_main_choice`,
                    lifespanCount: 1,
                }]
      ...
      return this.handleError(body, error, 'handleRestaurantDetails');
    }
  }
  ...
  async handleBookDineinPrompt(body: any) {
      try {
          ...
          if (!ctx || !ctx.parameters.restaurantId) {
              return {
                  fulfillmentText: `Sorry, I don't have enough information to proceed with booking. Type "Delivery", "Dine-in", "Track order/dine-in" or "Cancel" to continue.`,
                  outputContexts: [{
                      name: `${session}/contexts/awaiting_main_choice`,
                      lifespanCount: 1,
                  }]
          ...
          return this.handleError(body, error, 'handleBookDineinPrompt');
      }
  }
      ...
      try {
        ...
        Would you like to confirm the booking?`,
              outputContexts: [{
                  ...
                  lifespanCount: 1,
                  parameters: { cuisine, location, restaurantname, restaurantId, time, people, },
              }, {
                  name: `${session}/contexts/awaiting_main_choice`,
                  lifespanCount: 1,
              }],
          ...
          return this.handleError(body, error, 'handleConfirmBookingDetails');
      }
  }
  ...
  async handleDineInBooking(body: any) {
      try {
        ...
        Enjoy your meal! Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
          ...
          return this.handleError(body, error, 'handleDineInBooking');
      }
  }
  ...
  async handleTrackDineinOrder(body: any) {
    try {
      ...
      return this.handleError(body, err, 'handleTrackDineinOrder');
    }
  }
  ...
  async handleTrackOrder(body: any) {
    try {
      ...
      const fulfillmentText =
        `Here is the summary for your order #${orderId} from ${restaurantName}:\n\n` +
        `Status: ${status}\n` +
      ...
      return this.handleError(body, err, 'handleTrackOrder');
    }
  }
  ...
  private async handleFindRestaurant(body: any) {
    try {
      ...
      if (!location) {
        const locationCtx = body.queryResult.outputContexts.find(ctx => ctx.parameters?.location);
      ...
      if (!list.length) {
        return {
          fulfillmentText: `No ${cuisine} restaurants found in ${location}. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
          outputContexts: [{ name: `${session}/contexts/awaiting_main_choice`, lifespanCount: 1 }],
        };
      }
      ...
      return this.handleError(body, error, 'handleFindRestaurant');
    }
  }
  ...
  private async handleSelectDishes(body: any) {
    try {
      ...
      return this.handleError(body, error, 'handleSelectDishes');
    }
  }
  ...
  private async handleAddDish(body: any) {
    try {
      ...
      const { cuisine = '', location = '', restaurantName = '', restaurantId = '', dishes = [] } =
        dishCtx?.parameters || {};
      ...
      const addedList = dishNames.map((dish, idx) => `${quantities[idx] || 1} x ${dish}`).join(', ');
      const cartList = updatedDishes.map(d => `${d.quantity} x ${d.dishName}`).join(', ');
        ...
        outputContexts: [
          {
            name: `${session}/contexts/awaiting_dish_selection`,
            lifespanCount: 1,
            parameters: { cuisine, location, restaurantName, restaurantId, dishes: updatedDishes },
          },
        ],
      };
    } catch (error) {
      return this.handleError(body, error, 'handleAddDish');
    }
  }
  ...
  private async handleUpdateDishes(body: any) {
    try {
      ...
      if (!dishCtx) {
        return {
          fulfillmentText:
            "I couldn't find your current order to update. Please add/remove dishes or provide address.",
          outputContexts: [body.queryResult.outputContexts],
        };
      }

      let dishes = dishCtx.parameters?.dishes || [];
      const dishIndex = dishes.findIndex(d => d.dishName.toLowerCase() === dishName.toLowerCase());
      ...
      const cartSummary = dishes.map(d => `${d.quantity} × ${d.dishName}`).join(', ');
        ...
        outputContexts: [
          {
            name: `${session}/contexts/awaiting_dish_selection`,
            lifespanCount: 1,
            parameters: { ...dishCtx.parameters, dishes },
          },
        ],
      };
    } catch (error) {
      return this.handleError(body, error, 'handleUpdateDishes');
    }
  }
  ...
  private async handleRemoveDishes(body: any) {
    try {
      ...
      if (!dishName) {
        return {
          fulfillmentText: `I couldn't find the dish you want to remove. Please add/remove items or provide address.`,
          outputContexts: [body.queryResult.outputContexts],
        };
      }
      ...
      let dishes = [...(dishCtx.parameters?.dishes || [])];
      const index = dishes.findIndex(d => d.dishName.toLowerCase() === dishName.toLowerCase());
      ...
      return this.handleError(body, error, 'handleRemoveDishes');
    }
  }
  ...
  private async handleAddressConfirmation(body: any) {
    try {
      const session = body.session;
      const address =
        body.queryResult.parameters?.Address ||
        ...
        outputContexts: [
          {
            ...
            lifespanCount: 1,
            parameters: { ...dishCtx.parameters, Address: address },
          },
          { name: `${session}/contexts/awaiting_main_choice`, lifespanCount: 1 },
        ],
      };
    } catch (error) {
      return this.handleError(body, error, 'handleAddressConfirmation');
    }
  }
  ...
  private async handleConfirmOrder(body: any) {
    try {
      ...
      const { restaurantId, restaurantName, cuisine, location, dishes = [], Address } =
      ...
      const { orderId, totalPrice } = await this.orderService.placeOrder(1, restaurantId, items);
      const orderList = dishes.map((d: any) => `${d.quantity} × ${d.dishName}`).join(', ');
      ...
      return this.handleError(body, error, 'handleConfirmOrder');
    }
  }
  ...
  async handleCancelOrderConfirm(body: any) {
    try {
      ...
      if (!cancelCtx || !cancelCtx.parameters.number) {
        return {
          fulfillmentText:
            "Sorry, I didn't get the order ID to cancel. Please try again with your Order ID.",
          outputContexts: [
            { name: `${session}/contexts/awaiting_cancel_order_id`, lifespanCount: 1 },
          ],
        };
      }
      ...
      const updatedContext = {
        ...
        lifespanCount: 1,
        parameters: { ...cancelCtx.parameters, orderId },
      };
      ...
      return {
        ...
        outputContexts: [
          updatedContext,
          { name: `${session}/contexts/await_cancel_all`, lifespanCount: 1 },
        ],
      };
    } catch (error) {
      return this.handleError(body, error, 'handleCancelOrderConfirm');
    }
  }
  ...
  async handleCancelDineInConfirm(body: any) {
    try {
      ...
      if (!cancelCtx || !cancelCtx.parameters.number) {
        return {
          fulfillmentText:
            "Sorry, I didn't get the DineIn ID to cancel. Please try again with your DineIn ID.",
          outputContexts: [
            { name: `${session}/contexts/awaiting_cancel_dinein_id`, lifespanCount: 1 },
          ],
        };
      }
      ...
      if (!booking) {
        return {
          fulfillmentText: `Booking #${dineinId} not found. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
          outputContexts: [{ name: `${session}/contexts/awaiting_main_choice`, lifespanCount: 1 }],
        };
      }
      ...
      const updatedContext = {
        name: `${session}/contexts/awaiting_confirm_cancel`,
        lifespanCount: 1,
        parameters: { ...cancelCtx.parameters, dineinId },
      };
      ...
      return this.handleError(body, error, 'handleCancelDineInConfirm');
    }
  }
  ...
  async handleConfirmCancel(body: any) {
    try {
      ...
      if (!confirmCtx) {
        return {
          fulfillmentText: `Sorry, I couldn't find the confirmation context. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
          outputContexts: [{ name: `${session}/contexts/awaiting_main_choice`, lifespanCount: 1 }],
        };
      }
      ...
      if (!dineinId && !orderId) {
        return {
          fulfillmentText: `Your cancellation request has been processed successfully. Type "Delivery", "Dine‑in", "Track order/dine in" or "Cancel" to continue.`,
          outputContexts: [{ name: `${session}/contexts/awaiting_main_choice`, lifespanCount: 1 }],
        };
      }
      ...
      return this.handleError(body, err, 'handleConfirmCancel');
    }
  }
}